...
//                         <div
//                           className={`whitespace-pre-wrap rounded-2xl px-4 py-2 text-sm leading-relaxed border cursor-pointer ${
//                             m.role === "user"
...
export default function ChatbotUI(): JSX.Element {
  ...
  const callGeminiAPI = async (allMessages: ChatMessage[], pdfText: string): Promise<string> => {
    try {
      ...
      // Append PDF content only to the last user message
      if (pdfText && formattedMessages.length > 0) {
  ...
  const sendMessage = async (): Promise<void> => {
    ...
    const replyText = await callGeminiAPI([...messages, userMsg], parsedPDFText);
  ...
  const clearChat = (): void => {
    setMessages([]);
    setPending({ isThinking: false, placeholder: "" });
    setParsedPDFText(""); // clears all PDFs too
  };
  ...
  const handlePDFUpload = async (files: FileList) => {
    ...
    let combinedText = parsedPDFText; // <-- append new PDFs
